...
function SidebarItem({ icon, name , link, active}: Props) {
    return (
        ...
        >
            {icon}
            <span
                className="absolute left-full top-1/2 ml-4 -translate-y-1/2 rounded bg-sidebarSecondary text-sidebarPrimary px-2 py-1.5 text-xs font-medium text-white opacity-0 group-hover:opacity-100"
            >
                {name}
            </span>
        </Link>
